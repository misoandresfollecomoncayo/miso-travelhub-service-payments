...
# Cloud Tasks enqueue path is disabled — payments are now persisted inline.
...
async def payment_webhook(
    ...
    if payment is None:
        logger.info(
            "payment-webhook duplicate ignored tx=%s", payload.transactionId
        )
    else:
        logger.info(
...
# Worker endpoint kept commented for reference — no longer invoked, since
# /payment-webhook now persists synchronously.
