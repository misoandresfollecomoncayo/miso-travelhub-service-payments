...
class PaymentWebhookPayload(BaseModel):
    """Incoming webhook from the payments gateway.

    Field requirements depend on ``status``:

    - For ``REFUNDED`` only ``status``, ``message`` and ``invoiceId`` are
      required; the rest of the fields (amount/currency/card/transaction/
      processedAt) become optional because a refund event may be issued
      out-of-band without rebroadcasting the original transaction data.
